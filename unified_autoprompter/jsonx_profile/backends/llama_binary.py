...
VENDOR_ROOT = PACKAGE_ROOT / "vendor" / "unified-jsonx-llama.cpp"
USER_AGENT = "WorkflowX-Unified-JsonX"
...
def _paths(root: Path, spec: PlatformSpec) -> LlamaCliPaths:
    ...
    if cli is None:
        raise RuntimeError(f"Unified JsonX llama.cpp installation is missing {spec.cli_executable}: {root}")
