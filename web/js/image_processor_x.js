...
function button(ctx, rect, label, active = false, disabled = false) {
  ...
  ctx.fillStyle = disabled ? "#191c20" : active ? BUTTON_ACTIVE : "#202429";
  ctx.strokeStyle = disabled ? "#252a30" : active ? BUTTON_ACTIVE : "#3b424a";
  ctx.lineWidth = 1;
  ctx.fill();
  ctx.stroke();
...
function drawSelectButton(ctx, rect, label, active = false) {
  ...
  ctx.strokeStyle = active ? BUTTON_ACTIVE : "#3b424a";
  ctx.lineWidth = 1;
  ctx.fill();
  ctx.stroke();
...
function openEditor(node) {
  ...
  function drawBrushCursor(ctx) {
    ...
    const accent = s.tool === "eraser" ? "#ff6f61" : s.brushTarget === "adjustment" ? BUTTON_ACTIVE : GREEN;
