...
`Unified Autoprompter X` builds model-targeted prompt text from the WorkflowX autoprompting UI. Its `JsonX` target uses the same expandable **Model settings** provider surface as other targets while retaining independent JsonX provider state. The node's `ui_state` serializes the JsonX backend, selected provider models, portable runtime choices, and per-profile JsonX generation configuration. API keys, provider URLs or hosts, and absolute additional-model-folder paths remain browser- or machine-local. Use **Profile settings** and select a JsonX profile for Adaptive/Template Fill, preset context, framing map, hierarchy depth, output format, image-mode additions, instructions, and diagnostics. It mirrors the completed JsonX value to `prompt` and `positive`, and derives `negative` from the validated Stage 1 `negative` branch without changing other profiles.
...
`LLM to JsonX` generates prompt-only JsonX through Gemini, OpenAI-compatible, Ollama, or local GGUF backends. Provider settings and credentials use JsonX-specific browser storage and credentials are not serialized into workflows. Non-secret custom Stage 1, Template Fill, JSON-refinement, and natural-language instructions are saved in `ComfyUI/user/workflowx/jsonx_settings.json`.

Inputs include multiline `user_instructions`, `generation_mode` (`fast` or `refined`), `preset_context_mode` (`optimized` or `full`), UI-managed final-output storage, an optional `IMAGE`, and hidden per-node framing, output-format, generation-profile, Template Fill preset, and hierarchy-depth settings managed by Settings. Clicking `Generate` validates and saves the result in the node. The read-only output box mirrors the saved result, and the generic `prompt` output returns that same JSON or prose when queued; queueing does not call the LLM again. Optimized mode sends the complete preset path schema plus ranked values. Full mode sends the packaged preset file verbatim and may require a larger model context.
...
After parsing, canonicalization removes globally unique internal preset IDs from keys and values, restores known fields to their catalog paths, flattens preset ID/value leaf objects to scalar values, and normalizes singular `subject` to `subjects`. Compatible duplicate model forms resolving to one scalar leaf are joined deterministically; genuinely conflicting catalog choices or incompatible shapes trigger one repair call. A path-scoped alias table accepts only the former duplicate IDs in old/cached provider output; current contexts use the unique IDs exclusively. If repair also fails, the prior output remains unchanged and the node shows both raw responses in a transient diagnostics panel.

The default Adaptive/Deep contract targets granular parent/child/sub-child expansion across relevant scene, subject/object, lighting, camera, style, mood, and quality branches. The compact `Settings` button opens a modal with Adaptive and Template Fill profiles, the Template Fill `Use Presets` checkbox, the optional 3x3 framing map, per-node JSON/Natural output format, Deep and Exhaustive targets, editable JSON and prose backend instructions, default reset, and an exact effective-prompt preview. Adaptive/Template Fill, Use Presets, depth, framing, and output format are saved per node because they change the final contract. Custom instruction overrides are saved per ComfyUI user in `ComfyUI/user/workflowx/jsonx_settings.json`; provider configuration and credentials remain browser-local, and credentials remain excluded.

**Natural language** automatically synchronizes the node to Refined and uses exactly two normal calls for Adaptive or Template Fill. Stage 1 follows the selected profile and produces validated canonical JsonX. Stage 2 receives that in-memory object, original instructions, and optional image without presets, then improves coherence while converting all non-null details into prose. Top-level headings are allowed, nested JsonX keys and internal preset IDs are omitted, negative content becomes avoidance guidance, and enabled framing preserves all nine named regions. Only the final prose is saved; a failed conversion or single repair attempt leaves the previous output unchanged.
...
When **Framing & placement (3x3 rule-of-thirds map)** is enabled, both profiles must return a `framing_and_placement` object containing exactly nine non-empty scalar descriptions: `top_left`, `top_center`, `top_right`, `middle_left`, `center`, `middle_right`, `bottom_left`, `bottom_center`, and `bottom_right`. Empty visual areas describe background or negative space. The backend performs one same-provider repair for a missing or malformed map and keeps the previous output if repair fails. When disabled, the root is removed even if a provider emits it.
